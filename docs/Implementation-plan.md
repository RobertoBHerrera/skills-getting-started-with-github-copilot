...
| 12 | Actualizar datos embebidos en index.html | `index.html` | Frontend |
| 13 | Integración completa y test final | Todos | Validación |
...
**Tareas:**
- Importar `BaseModel`, `Field` de Pydantic y `datetime` de la librería estándar.
...
- Agregar estilos para estrellas con color dorado `#f4c430` o `#ffc107`.
...
## Paso 12: Actualizar datos embebidos en index.html

**Archivo:** `src/static/index.html`  
**Objetivo:** Incluir `average_rating` y `review_count` en los datos embebidos y agregar reviews iniciales de ejemplo.

**Tareas:**
- Actualizar el bloque `<script>` de `activitiesData` para incluir `average_rating` y `review_count` en cada actividad.
- Opcionalmente agregar un `reviewsData` embebido con reseñas de ejemplo para que la UI no esté vacía al cargar.
- Asegurar que el JavaScript lee estos datos correctamente.

**Criterio de éxito:**  
- La página carga con ratings de ejemplo visibles.
- Las estrellas se muestran en cada tarjeta desde el primer momento.

**Verificación:**  
```
Abrir la página → verificar que se ven estrellas y conteo de reviews
```

---

## Paso 13: Integración completa y test final
...
Paso 11 (Ordenamiento) ◄── depende de Paso 9
  │
  ▼
Paso 12 (Datos embebidos) ◄── depende de Paso 5 (campos nuevos definidos)
  │
  ▼
Paso 13 (Integración final) ◄── depende de TODOS los anteriores
```
