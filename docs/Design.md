...
│  (Vanilla JS, datos embebidos en HTML)           │
...
| `src/static/index.html` | Estructura HTML, datos embebidos en `<script>` como `activitiesData` | ~70 |
| `src/static/app.js` | Renderizado de tarjetas, formulario signup, botón unregister | ~140 |
...
El frontend actualmente **no hace fetch al API para cargar actividades**. Los datos están embebidos directamente en `index.html` como la variable global `activitiesData`. Sin embargo, las operaciones de escritura (signup, unregister) sí llaman al API. Las reseñas seguirán este patrón mixto: los datos se leen del API, las escrituras van al API.
...
- Actualizar el bloque `activitiesData` embebido para incluir `average_rating` y `review_count` (si se mantiene el patrón embebido) o migrar la carga de actividades al API.
...
| `src/static/index.html` | **Mínimo** — Agregar dropdown de ordenamiento, actualizar datos embebidos con campos nuevos | Bajo |
