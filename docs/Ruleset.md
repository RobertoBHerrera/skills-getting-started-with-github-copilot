...
- Respetar el patrón actual: datos iniciales embebidos en `index.html` como variables JavaScript globales.
...
### R-SEC-01: Validación de entrada en backend
- Todo input del usuario debe ser validado en el backend, no confiar en la validación del frontend.
- Usar modelos Pydantic para validar bodies JSON automáticamente.
- Validar query parameters manualmente con condiciones explícitas.

### R-SEC-02: Protección contra XSS
- En el frontend, al renderizar contenido dinámico del usuario (emails, comentarios de reseñas), utilizar `textContent` en lugar de `innerHTML` cuando sea posible.
- Si se usa `innerHTML`, sanitizar el contenido escapando caracteres HTML: `<`, `>`, `&`, `"`, `'`.
- Nunca insertar input de usuario directamente en template literals sin escapar.

### R-SEC-03: Protección contra inyección
- Dado que no hay base de datos, no hay riesgo de SQL injection.
- Sin embargo, validar que los strings no contengan intentos de script injection en comentarios.
- Limitar el tamaño de los inputs (email, comentario) para prevenir ataques de denegación de servicio por payload grande.

### R-SEC-04: Validación de email
- El email debe tener formato válido. Usar validación básica (contiene `@` y dominio).
- No se requiere verificación de que el email exista realmente (consistente con el sistema actual).
- El email es case-sensitive (consistente con el sistema actual).

### R-SEC-05: Rate limiting (no implementar, solo documentar)
- El sistema actual no tiene rate limiting. Esto es aceptable para un proyecto educativo.
- Si se necesitara en producción, se debería agregar limitación de requests por IP/email.
